...
STYLES_FILE = ROOT / "src" / "styles" / "styles.css"
APP_FILE = ROOT / "src" / "js" / "app.js"
CONFIG_FILE = ROOT / "src" / "site-config.json"
...
def resolve_target(source: pathlib.Path, raw_href: str) -> tuple[pathlib.Path | None, str]:
    ...
    if target.startswith("/"):
        clean = target.lstrip("/")
        if clean in {"styles.css", "app.js", "site-config.json"}:
            mapping = {
                "styles.css": STYLES_FILE,
                "app.js": APP_FILE,
                "site-config.json": CONFIG_FILE,
            }
            resolved = mapping[clean]
        elif clean.startswith("assets/"):
            resolved = ROOT / "src" / clean
        elif clean.endswith(".html"):
            resolved = PAGES_DIR / clean
        else:
            resolved = PUBLIC_DIR / clean
    else:
